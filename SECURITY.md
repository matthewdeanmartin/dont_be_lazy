# Security Policy for `dont_be_lazy`
...
1. **GitHub private vulnerability reporting** —
   [submit a confidential report](https://github.com/matthewdeanmartin/dont_be_lazy/security/advisories/new)
...
For maximum isolation, run `dont_be_lazy` inside a container so it only has access to what you
explicitly mount.
...
```bash
docker build -t dont_be_lazy .

# Mount the current directory into /data and pass it as an argument
docker run --rm -v "$(pwd):/data" dont_be_lazy /data
```
