...
PACKAGE := dont_be_lazy
PYTHON_TARGETS := dont_be_lazy tests
PYLINT_MAIN_TARGETS := dont_be_lazy
...
ABOUT_FILE := dont_be_lazy/__about__.py
...
help:
	...
	@echo "  dogfood                Run dont_be_lazy against this repo"
	@echo ""
...
dogfood:
	@echo "=== dont_be_lazy: repo summary ==="
	@$(UV) run dont_be_lazy --root . --no-color summary
	@echo "=== dont_be_lazy: package self-audit (high risk in shipped code) ==="
	@$(UV) run dont_be_lazy --root . --no-color scan $(PACKAGE) \
		--no-config-suppressions \
...
check: format-check lint-check security test typecheck metadata-check version-check dogfood
	@echo "All checks passed."

check-ci: format-check lint-check security test-ci typecheck metadata-check version-check dogfood
...
dont-be-lazy:
	...
	@$(UV) dont_be_lazy --root . --no-color scan dont_be_lazy --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@$(UV) pydoc_fork dont_be_lazy -o ./pydoc/
