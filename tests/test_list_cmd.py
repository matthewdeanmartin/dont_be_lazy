...
def test_cli_list_checks_all():
    result = subprocess.run(
        [sys.executable, "-m", "dont_be_lazy", "list", "checks"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_cli_list_checks_json():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "dont_be_lazy", "list", "checks", "--format", "json"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_cli_list_patterns():
    result = subprocess.run(
        [sys.executable, "-m", "dont_be_lazy", "list", "patterns", "--tool", "coverage"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_cli_list_only_inline():
    result = subprocess.run(
        [sys.executable, "-m", "dont_be_lazy", "list", "checks", "--only-inline"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
...
def test_cli_list_only_config():
    result = subprocess.run(
        [sys.executable, "-m", "dont_be_lazy", "list", "checks", "--only-config"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0
