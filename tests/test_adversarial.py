...
def _scan(source: str) -> list[Suppression]:
...
class TestFingerprintStability:
    def test_windows_vs_posix_path_same_fingerprint(self):
        ...
        a = _sup(path="src\\foo\\bar.py")
        b = _sup(path="src/foo/bar.py")
    ...
    def test_trailing_whitespace_normalized(self):
        ...
        a = _sup(text="import os  # noqa: F401   ")
        b = _sup(text="   import os  # noqa: F401")
        assert a.fingerprint() == b.fingerprint()
    ...
    def test_codes_sorted_in_fingerprint(self):
        ...
        a = _sup(codes=["F401", "E501"])
        b = _sup(codes=["E501", "F401"])
        assert a.fingerprint() == b.fingerprint()

    def test_different_tools_different_fingerprint(self):
        a = _sup(tool="ruff")
        b = _sup(tool="mypy", kind="type-ignore-specific")
        assert a.fingerprint() != b.fingerprint()

    def test_different_kind_different_fingerprint(self):
        a = _sup(kind="noqa-specific")
        b = _sup(kind="noqa-blanket", codes=[])
        assert a.fingerprint() != b.fingerprint()
    ...
    def test_deep_relative_path_normalised(self):
        ...
        a = _sup(path="./src/foo.py")
        b = _sup(path="src/foo.py")
...
class TestRiskLevelOrdering:
    ...
    def test_invalid_string_coerce_raises(self):
        with pytest.raises((ValueError, KeyError)):
            _ = RiskLevel.low < "extreme"  # type: ignore[operator]
...
class TestCommentScannerBoundary:
    ...
    def test_type_ignore_mixed_case(self):
        ...
        findings = _scan("x = 1  # Type: Ignore\n")
    ...
    def test_pragma_no_cover_mixed_case(self):
        findings = _scan("if x:  # Pragma: No Cover\n    pass\n")
    ...
    def test_pylint_disable_spaces_around_equals(self):
        findings = _scan("x = 1  # pylint: disable = missing-docstring\n")
    ...
    def test_noqa_and_type_ignore_same_line(self):
        ...
        findings = _scan("x: int = 1  # noqa: F401  # type: ignore\n")
    ...
    def test_nosec_multiple_codes(self):
        findings = _scan("x = exec(cmd)  # nosec B602, B603\n")
    ...
    def test_noqa_multiple_codes(self):
        findings = _scan("x = 1  # noqa: F401, E501\n")
...
class TestPathNormalization:
    def test_windows_absolute_path_in_suppression(self):
        s = _sup(path=r"C:\Users\dev\project\src\foo.py")
        assert s.id.startswith("DBL")

    def test_unc_path_doesnt_crash(self):
        s = _sup(path=r"\\server\share\project\foo.py")
        assert s.id.startswith("DBL")

    def test_path_with_spaces(self):
        a = _sup(path="C:/My Projects/src/foo.py")
        b = _sup(path=r"C:\My Projects\src\foo.py")
        assert a.fingerprint() == b.fingerprint()
...
class TestRiskScoring:
    def test_security_tool_bumps_risk(self):
        ...
        s = _sup(tool="bandit", kind="nosec-blanket", codes=[], reason=None, scope=ScopeKind.line)
        level = score(s)
        assert level >= RiskLevel.high

    def test_codes_credit_reduces_risk(self):
        """Having specific codes should credit (reduce) the computed risk."""
        without_codes = _sup(tool="ruff", kind="noqa-blanket", codes=[], reason=None)
        with_codes = _sup(tool="ruff", kind="noqa-specific", codes=["F401"], reason=None)
    ...
    def test_expiry_in_reason_credits_risk(self):
        ...
        without = _sup(tool="ruff", kind="noqa-specific", codes=["F401"], reason=None)
        with_expiry = _sup(
            tool="ruff",
            kind="noqa-specific",
            codes=["F401"],
    ...
    def test_file_scope_bumps_risk(self):
        line_s = _sup(tool="ruff", kind="noqa-specific", codes=["F401"], scope=ScopeKind.line)
        file_s = _sup(tool="ruff", kind="file-noqa", codes=[], scope=ScopeKind.file)
    ...
    def test_no_code_tools_not_penalized_for_missing_codes(self):
        ...
        s = _sup(tool="black", kind="fmt-skip", codes=[], reason=None, scope=ScopeKind.line)
    ...
    def test_unknown_tool_falls_back_to_medium_base(self):
        s = _sup(tool="novelscanner", kind="some-ignore", codes=[], reason=None)
        level = score(s)
        assert level in (RiskLevel.low, RiskLevel.medium, RiskLevel.high, RiskLevel.critical)

    def test_config_scope_bumps_risk(self):
        line_s = _sup(tool="ruff", kind="noqa-specific", codes=["F401"], scope=ScopeKind.line)
        cfg_s = _sup(tool="ruff", kind="config-ignore", codes=["F401"], scope=ScopeKind.config)
...
class TestPolicyEngine:
    ...
    def test_dbl001_not_fired_for_file_scope_blanket(self):
        ...
        s = _sup(flags=["blanket-ignore", "file-wide"], scope=ScopeKind.file)
        violations = check(s)
    ...
    def test_dbl006_security_tool(self):
        for tool in ("bandit", "semgrep", "secrets"):
            s = _sup(tool=tool, kind="nosec-blanket")
            violations = check(s)
