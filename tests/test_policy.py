...
def test_dbl002_file_wide():
    s = sup(scope=ScopeKind.file, flags=[])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl003_unclosed_block():
    s = sup(flags=["unclosed-block-suppression"], scope=ScopeKind.block)
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl004_not_triggered_when_reason_present():
    s = sup(reason="needed for legacy compat", flags=[])
    violations = check(s, policy={"require_reason": True})
    ids = [v.rule_id for v in violations]
...
def test_dbl006_security_tool():
    s = sup(tool="bandit", kind="nosec-blanket", flags=["blanket-ignore"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl007_type_checker():
    s = sup(tool="mypy", kind="type-ignore-blanket", flags=["blanket-ignore"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl008_skipped_test():
    s = sup(tool="pytest", kind="skip-unconditional", scope=ScopeKind.test, flags=[])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl009_nonstrict_xfail():
    s = sup(tool="pytest", kind="xfail-nonstrict", scope=ScopeKind.test, flags=[])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl010_config_ignore():
    s = sup(scope=ScopeKind.config, kind="config-ignore", flags=["config-level"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl011_config_exclude():
    s = sup(scope=ScopeKind.config, kind="config-exclude", flags=["config-level"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl013_unknown_suspicious():
    s = sup(tool="unknown", kind="suspicious-comment", flags=["suspicious"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_dbl015_vulnerability():
    s = sup(tool="safety", kind="ignored-vulnerability", scope=ScopeKind.config, flags=["security"])
    violations = check(s)
    ids = [v.rule_id for v in violations]
...
def test_check_all():
    findings = [
        sup(flags=["blanket-ignore"]),
        sup(tool="bandit", kind="nosec-blanket", flags=["blanket-ignore"]),
    ]
