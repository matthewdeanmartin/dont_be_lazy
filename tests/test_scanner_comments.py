...
def test_nosec_specific():
    findings = scan("x = bad()  # nosec B602\n")
...
def test_pragma_no_cover():
    findings = scan("if debug:  # pragma: no cover\n    pass\n")
    assert any(s.kind == "no-cover" for s in findings)


def test_pragma_no_branch():
    findings = scan("if x:  # pragma: no branch\n    pass\n")
...
def test_pylint_disable_all_critical():
    findings = scan("# pylint: disable=all\n")
...
def test_pyright_ignore_specific():
    findings = scan("x = 1  # pyright: ignore[reportUnknownMemberType]\n")
