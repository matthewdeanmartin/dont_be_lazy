import os
import pytest
from dont_be_lazy.config_loader import discover_config, _load_toml

def test_load_toml_invalid_path(tmp_path):
    # _load_toml is internal and expects the path to exist as checked by discover_config
    # but we can test it handles the case where it's called with something that fails open
    with pytest.raises(FileNotFoundError):
        _load_toml(str(tmp_path / "missing.toml"))
