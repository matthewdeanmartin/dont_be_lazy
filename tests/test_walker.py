...
def test_walks_python_files():
    root = make_tree({"src/a.py": "x=1", "src/b.txt": "nope"})
...
def test_excludes_venv():
    root = make_tree({".venv/lib/python.py": "x=1", "src/good.py": "y=2"})
    paths = list(walk_paths(root, respect_gitignore=False))
...
def test_exclude_glob():
    root = make_tree({"src/a.py": "x=1", "tests/t.py": "y=2"})
...
def test_walk_respect_gitignore(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dont_be_lazy.walker, "gitignored_files", lambda root: {"a.py"})
...
def test_gitignored_files_timeout(monkeypatch):
    ...
    assert gitignored_files(".") is None
...
def test_include_glob():
    root = make_tree({"src/a.py": "x=1", "other/b.py": "y=2"})
